...
struct LoadingBar: View {
    ...
    var body: some View {
        HeartIcon()
            .stroke(style: StrokeStyle(lineWidth: 5, lineCap: .round, lineJoin: .round, miterLimit: 0, dash: [150, 20], dashPhase: measuring ? -100 : 100))
            .frame(width: 100, height: 100)
            .foregroundColor(Color(.systemBlue))
            .hueRotation(.degrees(measuring ? -150 : 150))
            .onAppear{
                withAnimation(.linear(duration: 5).delay(-1).repeatForever(autoreverses: false).speed(2.5)) {
                    measuring.toggle()
                }
            }
    }
